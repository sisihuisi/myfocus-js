myFocus.pattern.extend({//*********************kendoui******************
	'mF_kendoui':function(settings,$){
		...
		$focus.find('.pic li').repeat();//无缝复制
		//CSS
		$picUl.css({width:settings.width*$txtList.length*2});
		//PLAY
		...
		$focus.play(null,function(i,n,l){
			...
			var index=i>=n?(i-n):i;
			var last=l>=n?(l-n):l;
			$txtList.eq(last).stop().slide({left:-settings.width},800,'easeInBack',function(){
				$txtList.eq(index).css({display:'block',left:settings.width}).slide({left:0},300);
				$picUl.slide({left:-settings.width*i},600,function(){param.isRunning=false});
				$dotList[last].className='';
